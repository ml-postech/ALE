...
def calculate_metric(
    ...
    # Base metrics on whole image
    if metric == "psnr":
        return metrics_calculator.calculate_psnr(pred_image, source_image, None, None)
    if metric == "lpips":
        return metrics_calculator.calculate_lpips(pred_image, source_image, None, None)
    if metric == "mse":
        return metrics_calculator.calculate_mse(pred_image, source_image, None, None)
    if metric == "ssim":
        return metrics_calculator.calculate_ssim(pred_image, source_image, None, None)
    if metric == "structure_distance":
        return metrics_calculator.calculate_structure_distance(
            pred_image, source_image, None, None
        )

    # Metrics on unedited parts (background)
    if metric == "psnr_unedit_part":
        if (1 - src_mask).sum() == 0 or (1 - tgt_mask).sum() == 0:
            return "nan"
        return metrics_calculator.calculate_psnr(
            pred_image, source_image, 1 - src_mask, 1 - tgt_mask
        )
    ...
    # Metrics on edited parts (foreground)
    if metric == "psnr_edit_part":
        if src_mask.sum() == 0 or tgt_mask.sum() == 0:
            return "nan"
        return metrics_calculator.calculate_psnr(
            pred_image, source_image, src_mask, tgt_mask
        )
    ...
    # ALE-specific metrics
    if metric == "TELS":
        # CLIP similarity between background (unedited) and target prompts (lower is better)
        tgt_prompt = " and ".join([t for _, t in edits_dict.items()])
        score = metrics_calculator.calculate_clip_similarity(
            pred_image, tgt_prompt, 1 - src_mask
        )
        return np.mean([score])

    if metric == "TILS":
        # Cross-target leakage: similarity of each target on other targets' regions (lower is better)
        if len(edits_dict) == 1:
            return "nan"
        used_mask_dict = {k: v for k, v in mask_dict.items() if k in edits_dict.keys()}
        clip_scores = []
        for src, target in edits_dict.items():
            # all masks except the source's own mask
            other_masks = {
                k: np.array(mask)[:, :, np.newaxis].repeat([3], axis=2) / 255.0
                for k, mask in used_mask_dict.items()
                if k != src
            }
            for _, mask in other_masks.items():
                score = metrics_calculator.calculate_clip_similarity(
                    pred_image, target, mask
                )
                clip_scores.append(score)
        return np.mean(clip_scores)

    if metric == "editing_performance":
        # CLIP similarity between edited region and the corresponding target prompt (higher is better)
        if len(edits_dict) == 1:
            tgt_prompt = " and ".join([t for _, t in edits_dict.items()])
            return metrics_calculator.calculate_clip_similarity(
                pred_image, tgt_prompt, tgt_mask
            )
        used_mask_dict = {k: v for k, v in mask_dict.items() if k in edits_dict.keys()}
        clip_scores = []
        for src, target in edits_dict.items():
    ...
    if metric == "clip_similarity_source_image":
        return metrics_calculator.calculate_clip_similarity(
            source_image, src_prompt, None
        )

    raise ValueError(f"Unknown metric: {metric}")


def save_image(np_img, filename, convert=True):
    if convert:
